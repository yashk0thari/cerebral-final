...
const openai = new OpenAI({
    apiKey: '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',
});
...
export async function POST(request: Request): Promise<NextResponse> {
    ...
    try {
        ...
        temperature: 0.3,
        messages: [
            { role: "system", content: `You are an expert TypeScript developer specializing in React and ShadCN UI components. Your task is to generate high-quality, visually appealing, customizable, and reusable TypeScript code for ShadCN UI components based on the user's prompt. Follow these guidelines:
...
Output only the JSX code for the App component, ready to be rendered. DO NOT GIVE ME ANY TEXT APART FROM THE CODE` },
            { role: "user", content: prompt }
        ],
        });
        const generatedCode = completion.choices[0].message?.content?.replace(/^```.*$/gm, '')
        .replace(/^import .*;$/gm, '')
        .trim()
         || '';
